...
def Newton(x0, tol, Nmax):
    for its in range(Nmax):
        ...
        try:
            ...
            print("Jacobian is singular at this point. Adjusting the guess.")
            x0 += np.random.uniform(-0.01, 0.01, size=x0.shape)
            continue
        Fn = F(x0)
        x1 = x0 - Jinv.dot(Fn)
        if norm(x1 - x0) < tol:
...
# Now, the code handles singular matrices by either adjusting the guess slightly or skipping if it can't proceed!
